...
void AAIPlayer::TickDecision()
{
	...
	TickSamples();
	// Ich mach das jetzt mal so, ich will jetzt nicht zu viel an fertiger logik ‰ndern
	currentTickStates = validStatesToTick;

	if(currentTickStates.Num() > 0)
	{
...
void AAIPlayer::SetDecisionTimer()
{
	...
	world->GetTimerManager().SetTimer(tickHandle, ticklambda, time, false);
}
